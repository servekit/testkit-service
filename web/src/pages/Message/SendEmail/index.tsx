 ...
 * recipients, one-line subject, WYSIWYG body editor, URL attachments, and an
 * advanced panel for the testkit-specific routing fields. Calls the GENERATED
 * sendEmail service — no hand-written fetch. `sender_id` is intentionally NOT
 * in the form: the BFF injects it from cfg.Message.SenderID. vendor/account
 * form a pair (both set or both empty — enforced by a CEL on the request and
 * mirrored here by making account required once a vendor is chosen).
 *
 * The plain-text body sent to message-service is auto-extracted from the HTML
 * content (or overridden in the advanced panel); message-service sends both as
...
export default function SendEmailPage() {
  ...
  const handleSend = async () => {
    ...
    const selectedVendor =
      vendor && vendor !== "EMAIL_VENDOR_UNSPECIFIED" ? vendor : undefined;
